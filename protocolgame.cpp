...
{
	NetworkMessage msg;
	msg.addByte(0x96);  // Opcode for sendTextWindow
	msg.add<uint32_t>(windowTextId);  // Add windowTextId

	// Add item details based on client version (assuming older client with item ID as uint16_t)
	msg.addItem(item);
	std::cout << "sendTextWindow: Opcode 0x96, windowTextId: " << windowTextId << std::endl;
	std::cout << "Item ID added: " << item << std::endl;

	msg.add<uint16_t>(maxlen);  // Add max length
	std::cout << "Max length added: " << maxlen << std::endl;

	std::string text = item->getText();
	if (!canWrite) {
		msg.add<uint16_t>(text.size());  // Add text size
	}
	else {
		msg.add<uint16_t>(0);  // Add zero length for writable text
	}
	msg.addString(text);  // Add text
	std::cout << "Text size added: " << text.size() << ", Text: " << text << std::endl;

	const std::string& writer = item->getWriter();
	if (!writer.empty()) {
		msg.addString(writer);  // Add writer
		std::cout << "Writer added: " << writer << std::endl;
	}
	else {
		msg.add<uint16_t>(0x00);  // Add empty writer placeholder
		std::cout << "No writer, added placeholder" << std::endl;
	}

	// Optional fields if necessary (assuming these features are not enabled for the old client)
	// if (g_game.getFeature(Otc::GameTibia12Protocol) && g_game.getProtocolVersion() > 1240)
	//     msg.add<uint8_t>(0x00);  // Placeholder for additional byte

	// if (g_game.getFeature(Otc::GameWritableDate))
	//     msg.addString("");  // Placeholder for date string


	writeToOutputBuffer(msg);
	std::cout << "Message written to output buffer." << std::endl;
}

void ProtocolGame::sendTextWindow(uint32_t windowTextId, uint32_t itemId, const std::string& text)
{
	NetworkMessage msg;
	msg.addByte(0x96); // OPCODE
	msg.add<uint32_t>(windowTextId); // TEXT ID

	// Add item details as itemId and count
	msg.add<uint16_t>(itemId); // For older clients, add itemId as uint16_t
	std::cout << "sendTextWindow: Opcode 0x96, windowTextId: " << windowTextId << ", itemId: " << itemId << std::endl;

	msg.add<uint16_t>(text.size()); // TEXT SIZE
	msg.addString(text); // TEXT
	std::cout << "Text size added: " << text.size() << ", Text: " << text << std::endl;

	msg.add<uint16_t>(0x00); // WRITER NAME placeholder
	std::cout << "No writer, added placeholder" << std::endl;

...
void LuaScriptInterface::registerFunctions()
{
	...
	//isRotatable(uid)
	lua_register(luaState, "isRotatable", LuaScriptInterface::luaIsRotatable);

	//doRotateItem(uid)
	lua_register(luaState, "doRotateItem", LuaScriptInterface::luaDoRotateItem);
	...
	//NEWSKILL
	registerEnum(CONDITION_PARAM_SKILL_FARMING)
	...
	//NEWSKILL
	registerEnum(CONDITION_PARAM_SKILL_FARMINGPERCENT)
...
//
int LuaScriptInterface::luaIsRotatable(lua_State* L) {
	// isRotatable(uid)
	uint32_t uid = getNumber<uint32_t>(L, -1);
	Item* item = getScriptEnv()->getItemByUID(uid);
	pushBoolean(L, item && item->isRotatable());
	return 1;
}

int LuaScriptInterface::luaDoRotateItem(lua_State* L) {
	// Get parameters from the Lua stack
	...
	// Get the item by UID
	Item* item = getScriptEnv()->getItemByUID(uid);
	if (!item || !item->isRotatable() || item->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID)) {

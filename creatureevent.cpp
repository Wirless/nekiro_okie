...
//onGainskill
bool CreatureEvents::playerProgress(Player* player, skills_t skill, uint64_t tries)
{
	for (auto& it : creatureEvents) {
		if (it.second.getEventType() == CREATURE_EVENT_ONPROGRESS) {
			if (!it.second.executeProgress(player, skill, tries)) {
				return false;
			}
		}
	}
...
bool CreatureEvent::configureEvent(const pugi::xml_node& node)
{
	...
	else if (tmpStr == "progress") {
		type = CREATURE_EVENT_ONPROGRESS;
	}
...
bool CreatureEvent::executeProgress(Player* player, skills_t skill, uint64_t tries)
{
	...
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeProgress] Call stack overflow" << std::endl;
		return false;
	}
